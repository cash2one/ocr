<div class="top-section">
    <div class="top-pic">
            <a class="btn-bg-blue"
                style="position: absolute; height: 45px; width: 140px; left: 760px; top: 352px;"
            href="http://hetu.baidu.com/api/?categoryId=186" target="_blank"></a>
    </div>
</div>
<div class="intro-section" id="intro">
    <div class="intro-pic">
        <a href="http://session.baidu.com" target="_blank">
            <div class="intro-detail" 
                style="left: 84px;"
            >查看详情</div>
        </a>
        <a href="http://session.baidu.com/#pc_experience" target="_blank">
            <div class="intro-detail"
                style="left: 174px;"
            >技术体验</div>
        </a>
        <a href="http://offlinedata.baidu.com/" target="_blank">
            <div class="intro-detail"
                style="left: 513px;"
            >查看详情</div>
        </a>
        <a href="http://offlinedata.baidu.com/introduce" target="_blank">
            <div class="intro-detail"
                style="left: 603px;"
            >技术体验</div>
        </a>
        <a href="http://gravity.baidu.com/" target="_blank">
            <div class="intro-detail"
                style="left: 945px;"
            >查看详情</div>
        </a>
        <a href="http://gravity.baidu.com/index.html#!/sample/" target="_blank">
            <div class="intro-detail"
                style="left: 1035px;"
            >技术体验</div>
        </a>

    </div>
</div>
<div class="sep-line"></div>
<div class="advan-section">
    <div class="advan-pic">
    </div>
</div>
<div class="sep-line"></div>
<div class="app-case"></div>
<div class="product-section">
    <div class="product-pic" style="overflow: hidden;">
        <ul class="scroll">
            <li class="slide-1"></li>
            <li class="slide-2"></li>
            <li class="slide-3"></li>
            <li class="slide-1"></li>
        </ul>
        <ul class="scroll-btn">
            <li class="tip-btn">个性化广告</li>
            <li class="tip-btn">个性化排序及推荐</li>
            <li class="tip-btn">APP广告分发优化</li>
        </ul>
    </div>
</div>
<!--
<div class="experience-section">
    <div class="experience-pic">
        <a href="http://yuyin.baidu.com/#try" target="_blank">
            <div class="exp-detail exp-vr"></div>
            <div class="exp-detail exp-vr-hover"><span class="magic-bar"></span>语音识别</div>
        </a>
        <a href="http://yuyin.baidu.com/#try" target="_blank">
            <div class="exp-detail exp-vc"></div>
            <div class="exp-detail exp-vc-hover"><span class="magic-bar"></span>语音合成</div>
        </a>
        <div class="exp-detail exp-vwr"></div>
    </div>
</div>
<div class="api-section">
    <div class="api-pic">
        <a href="http://hetu.baidu.com/api/platform/index?platformId=1" target="_blank">
            <div class="api-detail api-vr"></div>
        </a>
        <a href="http://hetu.baidu.com/api/platform/index?platformId=754" target="_blank">
            <div class="api-detail api-vc"></div>
        </a>
        <div class="api-detail api-vwr"></div>
    </div>
</div>
-->
<div class="contact-section" id="contact-us">
    <div class="contact-pic">
        <a href="baidu://addgroup/?id=1516306">
            <div class="hi contact">1516306</div>
        </a>
        <a href="mailto:ai-bp@baidu.com">
            <div class="mail contact">ai-bp@baidu.com</div>
        </a>
        <a href="http://hetu.baidu.com/api/area/ai" target="_blank">
            <div class="hetu contact">河图AI专区</div>
        </a>
    </div>
</div>
{%script%}
    var bd = require('brain:widget/bd/bd.js');
    bd.init();
{%/script%}
