...
// 黄反接口url
const static std::string antiporn_url = "https://aip.baidubce.com/rest/2.0/antiporn/v1/detect";
static std::string antiporn_result;
/**
...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    ...
    antiporn_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
/**
 ...
 */
int antiporn_detect(std::string &json_result, const std::string &image_base64, const std::string &access_token) {
    std::string url = antiporn_url + "?access_token=" + access_token;
    CURL *curl = NULL;
