...
// 银行卡ocr接口url
const static std::string bank_card_url = "https://aip.baidubce.com/rest/2.0/ocr/v1/bankcard";
static std::string bank_card_result;
/**
...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    // 获取到的body存放在参数ptr中，先将其转换为string格式
    bank_card_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
/**
 ...
 */
int bank_card_recognize(std::string &json_result, const std::string &image_base64,
                                   const std::string &access_token){
    std::string url = bank_card_url + "?access_token=" + access_token;
    CURL *curl = NULL;
    CURLcode result_code;
    int is_success;
    curl = curl_easy_init();
    if (curl) {
        // 使用libcurl post数据：设定待post的url，将image以表单的形式填入body
