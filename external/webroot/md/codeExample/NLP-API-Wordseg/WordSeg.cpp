...
// 人脸检测接口url
const static std::string word_seg_url = "https://aip.baidubce.com/rpc/2.0/nlp/v1/wordseg";
static std::string word_seg_result;
...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    ...
    word_seg_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
 ...
 */
int word_seg(std::string &json_result, const std::string &query, const std::string &access_token) {
    std::string url = word_seg_url + "?access_token=" + access_token;
    CURL *curl = NULL;
