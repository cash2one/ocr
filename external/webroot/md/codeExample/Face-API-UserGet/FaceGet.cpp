...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    ...
    get_user_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
 ...
 */
int get_user(std::string &json_result, const std::string &uid,
             const std::string &access_token) {
    std::string url = get_user_url + "?access_token=" + access_token;
    CURL *curl = NULL;
