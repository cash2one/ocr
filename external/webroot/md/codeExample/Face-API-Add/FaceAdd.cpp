...
// 接口url
const static std::string add_user_url = "https://aip.baidubce.com/rest/2.0/face/v2/faceset/user/add";
static std::string add_user_result;
...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    ...
    add_user_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
...
int add_user(std::string &json_result, const std::string &uid, const std::string &user_info,
    ...
    std::string url = add_user_url + "?access_token=" + access_token;
    CURL *curl = NULL;
    ...
    if (curl) {
        ...
        curl_formadd(&post, &last, CURLFORM_COPYNAME, "uid", CURLFORM_COPYCONTENTS, uid.data(),
                     CURLFORM_END);
        curl_formadd(&post, &last, CURLFORM_COPYNAME, "user_info", CURLFORM_COPYCONTENTS, user_info.data(),
                     CURLFORM_END);
