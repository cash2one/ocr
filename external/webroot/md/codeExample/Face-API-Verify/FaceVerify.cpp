...
static size_t callback(void *ptr, size_t size, size_t nmemb, void *stream) {
    ...
    verify_result = std::string((char *) ptr, size * nmemb);
    return size * nmemb;
}
 ...
 */
int verify(std::string &json_result, const std::string &uid, const std::vector<std::string> &images,
           const std::string &access_token, int top_num) {
    std::string url = verify_url + "?access_token=" + access_token;
    CURL *curl = NULL;
    ...
    if (curl) {
        ...
        curl_formadd(&post, &last, CURLFORM_COPYNAME, "images", CURLFORM_COPYCONTENTS, image_plain.data(),
                     CURLFORM_END);
        curl_formadd(&post, &last, CURLFORM_COPYNAME, "top_num", CURLFORM_COPYCONTENTS, int_to_string(top_num).data(),
                     CURLFORM_END);
