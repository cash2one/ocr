...
分词接口提供基本词和混排两种粒度的分词结果，基本词粒度较小，适用于搜索引擎等需要更多召回的任务，而混排粒度倾向于保留更多的短语。分词接口将逐步下线，请移步使用[词法分析接口](#词法分析接口)
...
词性标注接口为分词结果中的每个单词标注一个正确的词性的程序，也标注每个词是名词、动词、形容词或其他词性。词性标注接口将逐步下线，请移步使用[词法分析接口](#词法分析接口)
...
```
POST /rpc/2.0/nlp/v1/lexer HTTP/1.1
accept-encoding: gzip, deflate
x-bce-date: {utc-date-string}
connection: keep-alive
accept: */*
...
| **参数名称** | **类型**           | **详细说明**           |
| -------- | ---------------- | ------------------ |
| texts    | array of strings | 待分析文本集合，支持批量请求多个文本 |
...
| ---------- | ---- | ------------- |
| staus=0    | 正常   | 正常            |
| status=401 | 部分正常 | 部分返回正常，部分返回失败 |
| status=402 | 算法失败 | 算法异常          |
...
| ----------- | ---- | ------------------- |
| retcode=0   | 正常   | 正常                  |
| retcode=401 | 输入太长 | 输入的请求太长 （最长65536字节） |
| retcode=402 | 编码错误 | 输入请求编码错误 （要求GB编码）   |
| retcode=403 | 算法错误 | 算法异常， 需要联系客服排查      |
