...
/***/ function(module, exports, __webpack_require__) {
	...
	var matchAnchor = function matchAnchor(className, cnName) {
	...
	var setAnchorId = function setAnchorId(arr) {
	    if (!arr.length) {
	        return;
	    }
	    for (var i = 0; i < arr.length; i++) {
	        (0, _jquery2.default)(arr[i]).attr('id', matchAnchor('faceRecognition', (0, _jquery2.default)(arr[i]).text()));
	    }
	};

	var setFaqAnchorId = function setFaqAnchorId(tagname) {
	    if (tagname.indexOf('FAQ') > 0) {
	        (0, _jquery2.default)('#md_container p').each(function (i, element) {
	            // $(this).attr('id', tagname + '-Q' + (i + 1));
	            (0, _jquery2.default)(this).attr('id', 'Q' + (i + 1));
	        });
	    }
	};

	var bindLeafNodeScroll = function bindLeafNodeScroll(clickNode, type) {
	    ...
	    leafNodes.each(function (i, element) {
	        (0, _jquery2.default)(element).click(function () {
	            var curTag = (0, _jquery2.default)(this).parent().parent().find('>a').attr('tag');
	            if (curTag === lastMdTag) {
	                scrollToLeafNodeH1(i);
	            } else {
	                renderMdPage(curTag, (0, _jquery2.default)(this), type);
	            }
	        });
	    });
	};

	var renderALinkTag = function renderALinkTag() {
	    (0, _jquery2.default)('#md_container h1, #md_container h2').each(function (i, element) {
	        (0, _jquery2.default)(element).attr('id', (0, _jquery2.default)(element).text());
	    });
	    var aList = (0, _jquery2.default)('#md_container a');
	    aList.each(function (i, element) {
	        var aTag = (0, _jquery2.default)(element);
	...
	var renderMdPage = function renderMdPage(tagName, clickNode, type) {
	    ...
	    _jquery2.default.ajax({
	        ...
	        success: function success(res) {
	            ...
	            (0, _jquery2.default)('#md_container').scrollTop(0);
	            renderALinkTag();
	...
	var bindMinusPlus = function bindMinusPlus() {
	    (0, _jquery2.default)('.sidebar > h1').click(function () {
	        var button = (0, _jquery2.default)(this).find('.pm-button');
	        if (button.hasClass('nav-plus1') && button.hasClass('active')) {
	            ...
	            button.removeClass('active');
	            (0, _jquery2.default)('.toc.jquery-accordion-menu:eq(1) > ul').show(500);
	        } else {
	            button.addClass('active');
	            (0, _jquery2.default)('.toc.jquery-accordion-menu:eq(1) > ul').hide(500);
	        }
	    });
	};

	var loadDefault = function loadDefault() {
	    (0, _jquery2.default)('.doc-wrap .beginner > li:eq(0)').click();
	};

	var clickonce = true;
	var unfoldSidebar = function unfoldSidebar(id) {
	    var element = (0, _jquery2.default)('[name="' + id + '"]');
	    var parentUl = element.parent();
	    var parentLi = parentUl.parent();
	    var firstClickNode = element.find('.click-node:eq(0)');
	    var clickElement = function clickElement() {
	        if (clickonce) {
	            element.find('>a').click();
	            firstClickNode.click();
	            var subUl = firstClickNode.siblings().eq(0);
	            if (subUl.length) {
	    ...
	    if (parentLi.attr('id') === 'jquery-accordion-menu') {
	        (0, _jquery2.default)('.pm-button:eq(1)').removeClass('active');
	    }
	    unfoldSidebar(parentLi.attr('name'));
	};

	var loadHashLocation = function loadHashLocation() {
	    ...
	    if (hashId.split('_').length === 1) {
	        ...
	        (function () {
	            var hash = hashId.split('_')[0];
	            var questionId = hashId.split('_')[1];
	            // questionId = hash + '-' + questionId;
	            unfoldSidebar(hash);
	            setTimeout(function () {
	                var questionElement = (0, _jquery2.default)('#md_container>#' + questionId);
	                if (questionElement.length) {
	                    var offset = questionElement.offset().top;
	                    (0, _jquery2.default)('body').scrollTop(offset);
	                }
	            }, 500);
	        })();
	    }
