    ...
    var startScan = function startScan(type, imgSrc) {
        ...
        var options = {
            success: function success(res) {
                ...
                if (res.errno !== 0) {
                    (0, _jquery2.default)('#demo-result .result-background').toggleClass('has-result man female', false).toggleClass('error-upload-fail', res.errno === 1).toggleClass('error-timeout', res.errno === 28).toggleClass('error-image-format', res.errno === 216201);
                    isScanning = false;
                    return false;
                }
                ...
                for (var key in res.data.words_result) {
                    ...
                    var words = res.data.words_result[key].words;
                    hasNoResult = hasNoResult && !words;
                    (0, _jquery2.default)('#demo-result .result-background').find('.' + ID_CARD_KEY_MAP[key]).html(words);
                    if (key === '性别') {
                        (0, _jquery2.default)('#demo-result .result-background').toggleClass(ID_CARD_KEY_MAP[words] || '', true);
                    }
                }
